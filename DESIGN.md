...
title: "Owen Exporter Plugin Design"
status: adopted
product: "owen-exporter"
surface: "Obsidian export settings, progress and preview modals"
kind: plugin
updated: 2026-08-12
...
# Owen Exporter Plugin Design

## Decision Summary

| 항목 | 결정 |
| --- | --- |
| 사용자 | Obsidian 문서·SVG를 web, image, standalone HTML로 내보내는 작성자 |
| Output job | 선택한 source를 원하는 format·quality·asset policy로 export하고 결과를 검증한다 |
| Primary object | SVG, Markdown selection, export profile과 output artifact |
| Primary action | Export 설정을 검토하고 file 또는 clipboard output을 생성한다 |
| Composition | SettingsPanel + bounded progress modal + opaque comparison preview |
| 핵심 판단 | Export 결과의 fidelity와 validation을 visual flourish보다 우선한다 |

## Context

Owen Exporter는 SVG raster export와 Markdown HTML export를 제공한다. Image format·scale·quality, HTML profile·asset mode, recent history와 batch progress가 주요 state다.
...
- AGENTS.md
- package.json
- export sample validation

## Foundation Selection

### Adopted

- SettingsPanel로 image, HTML, asset, history, validation 구분
- Progress row에 pending·running·success·failed·skipped text/icon 제공
- Comparison preview는 opaque side-by-side surface
- Native file dialog와 Obsidian modal semantics 유지

### Excluded Or App-Owned

- Export preview의 decorative glass frame
- 실패를 전체 batch 실패로 표현
- 검증 전 success toast
...
- **결정**: Export profile은 settings에서 저장하고 실행 modal은 이번 output의 delta만 보여준다
- **이유**: 반복 export에서 같은 설정을 매번 다시 읽지 않게 한다
- **검증**: Saved setting load와 one-run override test를 수행한다

### D-02. Batch progress는 row state를 유지한다

- **결정**: 각 file의 pending, running, success, failed, skipped와 manifest path를 표시한다
- **이유**: Partial success를 숨기지 않고 실패 file만 재시도해야 한다
- **검증**: Mixed outcome fixture와 retry flow를 검증한다

### D-03. HTML profile 비교는 content fidelity 중심이다

- **결정**: Obsidian-like, portable, clean 결과를 같은 width와 sample에서 나란히 비교한다
- **이유**: Style 차이보다 table, callout, code, image 보존 여부를 판단해야 한다
- **검증**: Canonical sample document와 asset mode별 output을 비교한다
...
- SVG dimension·path·filename unit tests
- HTML sample fidelity and broken asset validation
